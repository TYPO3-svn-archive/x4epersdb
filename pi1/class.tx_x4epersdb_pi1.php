...
class tx_x4epersdb_pi1 extends x4epibase {
		// Same as class name
	var $prefixId = 'tx_x4epersdb_pi1';
		// Path to this script relative to the extension dir.
	var $scriptRelPath = 'pi1/class.tx_x4epersdb_pi1.php';
		// The extension key.
	var $extKey = 'x4epersdb';
	...
	function main($content, $conf) {
		...
		$this->pi_loadLL();
			...
			// TODO: clean up and correct source for options -> typoscript!
	...
	function listViewByDepartment() {
		$addWhere = ($this->getTSFFvar('departmentUids') != '') ? ' AND uid IN('.$this->getTSFFvar('departmentUids').')' : '';
		$departments = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', $this->departmentTable, 'pid IN ('.$this->conf['feUsers']['pidList'].')'.$addWhere.$this->cObj->enableFields($this->departmentTable), '', 'title ASC');
		...
		switch($fN) {
			...
			case 'function':
				...
				if ($this->internal['currentRow'][$fN] != '') {
					$f = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', $this->functionTable,'uid IN ('.$this->internal['currentRow'][$fN].')'.$this->cObj->enableFields($this->functionTable));
					$funcs = array();
	...
	function pi_list_row($c) {
		...
		foreach ($this->manualFieldOrder_list as $fieldName) {
			...
			} elseif ($fieldName == 'function_suffix') {
